...
static cl::opt<bool> PrintInsts("print-insts", cl::desc("Print the ID-instruction mapping"));
static cl::opt<bool> PrintValues("print-values", cl::desc("Print the ID-value mapping"));
...
const unsigned IDAssigner::InvalidID = -1;
...
void IDAssigner::printInstructions(raw_ostream &O, const Module *M) const
{
  ...
  std::vector<std::pair<unsigned, const Instruction *> > All;
...
void IDAssigner::printValues(raw_ostream &O, const Module *M) const
{
  ...
  std::vector<std::pair<unsigned, const Value *> > All;
...
static RegisterPass<IDAssigner> X("assign-id", "Assign a unique ID to each instruction and each value", false, true);
